    ...
    <div class="w3-main w3-animate-left" id="main" style="margin-left:300px;margin-top:43px;">
        ...
        <ul class="w3-ul w3-card-4 w3-white">
            <?php foreach ($list['pending'] as $pending) { ?>
                <li class="w3-padding-16" id="lista<?php echo esc($pending['id']); ?>">
                    <img src="<?php base_url(); ?><?php echo $pending['avatar']; ?>"
                        class="w3-left w3-circle w3-margin-right" style="width:35px">
                    <span class="w3-xlarge">
                        <?php echo esc($pending['surname']); ?>
                        <?php echo esc($pending['first_name']); ?>
                        <?php echo esc($pending['address']); ?>
                        <?php echo esc($pending['City_of_residence']); ?>
                        <?php echo esc($pending['zip_code']); ?>
                        <?php echo esc($pending['Province_of_residence']); ?>
                        <?php echo esc($pending['phone_number']); ?>
                        <?php echo esc($pending['user_type']); ?>
                    </span>
                    <button class="w3-btn w3-white w3-border w3-border-green w3-round-large" style="float:right"
                        onclick="activeuser(<?php echo esc($pending['id']); ?>)"
                        data-val="<?php echo esc($pending['id']); ?>"><?php echo lang('Auth.btnattivautente'); ?></button>
                    <br>
                </li>
            <?php } ?>
        </ul>
